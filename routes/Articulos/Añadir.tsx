...
import { ArticuloAdd } from "../../components/Articulos_V2/ArticuloAdd.tsx";
...
export const handler: Handlers<unknown, MyState> = {
  ...
  async POST(req: Request, ctx: FreshContext<MyState>) {
    ...
    if (id) {
      ...
      const [cont] = await (
        await db()!
      ).query(`SELECT * FROM fabrica.contacto WHERE id_cliente=${id} `);
      const [dir] = await (
        await db()!
      ).query(`SELECT * FROM fabrica.direccion WHERE id_cliente=${id} `);
      const [em] = await (
        await db()!
      ).query(`SELECT * FROM fabrica.empresa WHERE id_cliente=${id} `);
...
export default function Home(props: PageProps<unknown, MyState>) {
  return (
